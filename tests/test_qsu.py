...
import io
import os
import pathlib # For type hinting and spec, not for direct use in qsu.py's context
...
class TestQsu(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_script_dir_obj.__fspath__ = MagicMock(return_value=self.mock_app_dir_str)
    ...
    def test_successful_component_start(self, mock_ArgumentParser_constructor, mock_Path_class_in_qsu, mock_subprocess_run, mock_stdout):
        ...
        mock_script_parent_obj.resolve = MagicMock(return_value=self.mock_script_dir_obj)
        ...
        mock_comp_home_obj.__fspath__ = MagicMock(return_value=comp_home_path_str) # For cwd argument
        mock_comp_home_obj.__str__ = MagicMock(return_value=comp_home_path_str) # For string interpolation
        self.mock_script_dir_obj.__truediv__ = MagicMock(return_value=mock_comp_home_obj)
        ...
        mock_start_script_obj.__fspath__ = MagicMock(return_value=start_script_full_path_str)
        mock_start_script_obj.__str__ = MagicMock(return_value=start_script_full_path_str) # Consistent str
        ...
        mock_subprocess_run.return_value = subprocess.CompletedProcess(args=["./start.sh"], returncode=0)
        ...
        mock_subprocess_run.assert_called_once_with(["./start.sh"], check=True, cwd=mock_comp_home_obj)
    ...
    def test_non_existent_component_directory(self, mock_ArgumentParser_constructor, mock_Path_class_in_qsu, mock_subprocess_run, mock_stdout):
        ...
        mock_comp_home_obj.__truediv__ = MagicMock(return_value=mock_start_script_obj) # For start_script_path

        mock_comp_home_obj.is_dir.return_value = False # Key for this test scenario

        qsu.main()

        self.assertEqual(f"no component {component_name}", mock_stdout.getvalue().strip())
    ...
    @patch('qsu.pathlib.Path')
    @patch('qsu.argparse.ArgumentParser')
    def test_start_sh_missing(self, mock_ArgumentParser_constructor, mock_Path_class_in_qsu, mock_subprocess_run, mock_stdout):
    ...
    @patch('qsu.pathlib.Path')
    @patch('qsu.argparse.ArgumentParser')
    def test_execution_fails_called_process_error(self, mock_ArgumentParser_constructor, mock_Path_class_in_qsu, mock_subprocess_run, mock_stdout):
        ...
        output_lines = mock_stdout.getvalue().splitlines()
        expected_error_message = f"Error starting component {component_name}: {error_obj}"
    ...
    @patch('qsu.pathlib.Path')
    @patch('qsu.argparse.ArgumentParser')
    def test_start_sh_not_found_by_subprocess(self, mock_ArgumentParser_constructor, mock_Path_class_in_qsu, mock_subprocess_run, mock_stdout):
        ...
        comp_home_path_str = os.path.join(self.mock_app_dir_str, component_name)
        mock_comp_home_obj.__fspath__ = MagicMock(return_value=comp_home_path_str)
        mock_comp_home_obj.__str__ = MagicMock(return_value=comp_home_path_str) # For the {comp_home} part of the message
        self.mock_script_dir_obj.__truediv__ = MagicMock(return_value=mock_comp_home_obj)
        ...
        mock_start_script_obj.__str__ = MagicMock(return_value="./start.sh") # <--- THE FIX
        mock_comp_home_obj.__truediv__ = MagicMock(return_value=mock_start_script_obj)

        mock_comp_home_obj.is_dir.return_value = True
        mock_start_script_obj.is_file.return_value = True # Script file exists by initial check
        
        mock_subprocess_run.side_effect = FileNotFoundError("script not found by subprocess.run mock")
        ...
        expected_script_name_in_msg = "./start.sh" # This is what str(mock_start_script_obj) will return
        ...
        self.assertIn(f"starting {component_name} ...", output_lines)
        self.assertIn(expected_error_message, output_lines, f"Expected error message not found. Full output: {output_lines}")
        mock_subprocess_run.assert_called_once_with(["./start.sh"], check=True, cwd=mock_comp_home_obj)
