...
class ExtractedDependency(BaseModel):
    ...
    type: Optional[str] = Field(None, description="e.g., 'OS', 'language', 'library', 'tool'")

class ExtractedStep(BaseModel):
    description: str
    command: Optional[str] = None
    type: str = Field(..., description="e.g., 'download', 'configure', 'build', 'run', 'verify'")
...
class ExtractedContent(BaseModel):
    ...
    raw_text_summary: Optional[str] = Field(None, description="A brief summary if generated by AI")
    ...
    extraction_metadata: Dict[str, Any] = Field(default_factory=dict, description="e.g., AI model used, confidence")
...
class MockContentExtractor(ContentExtractor):
    async def extract(self, html_content: str, url: HttpUrl) -> ExtractedContent:
        ...
        if "nginx" in html_content.lower():
            ...
            docker_run_command = "docker run -d -p 8080:80 --name my-nginx nginx # (mock extracted)"
            dependencies.append(ExtractedDependency(name="Docker", type="tool"))
            setup_steps.append(ExtractedStep(description="Pull Nginx Docker image", command="docker pull nginx", type="download"))
            setup_steps.append(ExtractedStep(description="Run Nginx Docker container", command=docker_run_command, type="run"))
...
services:
  nginx-mock:
    ...
    restart: unless-stopped'''
            ...
            extraction_metadata = {"source": "MockExtractor - No specific tool recognized"}
            setup_steps.append(ExtractedStep(description="No specific tool recognized by mock extractor.", type="info"))
