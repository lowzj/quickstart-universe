...
app = FastAPI()
templates = Jinja2Templates(directory="app/templates") # Assumes templates are in 'app/templates'
...
async def process_url(request: Request, url: str = Form(...)):
    ...
    try:
        ...
        response = requests.get(url, timeout=10, headers={'User-Agent': 'QuickstartUniverseMVP/0.1'})
        ...
        try:
            ...
            return templates.TemplateResponse("index.html", {
                "request": request, "submitted_url": url, "error": error_message, 
                "raw_content_snippet": raw_content_snippet, "simulated_steps": simulated_steps
            })

        # Call the content extractor
        simulated_steps.append(f"Calling MockContentExtractor for URL: {pydantic_url_obj}")
        extracted_data: ExtractedContent = await extractor.extract(html_content=html_content, url=pydantic_url_obj)
        extracted_info = extracted_data.model_dump(exclude_none=True) # Convert Pydantic model to dict
        simulated_steps.append(f"MockContentExtractor finished. Tool identified: {extracted_data.tool_name or 'None'}")
...
echo "{extracted_data.tool_name or 'Tool'} container should be running. Check with 'docker ps'."'''
        ...
        if extracted_data.docker_compose_snippet:
            ...
            docker_compose_script = "# No Docker Compose snippet suggested by extractor."
        ...
        if not extracted_data.tool_name and extracted_info.get("extraction_metadata", {}).get("source") == "MockExtractor - No specific tool recognized":
            if "message" not in extracted_info: 
                 extracted_info["message"] = "No specific tool recognized by content extractor."

    except requests.Timeout:
        logger.error(f"Timeout when fetching URL: {url}")
        error_message = "The request timed out. The server might be slow or the URL incorrect."
        simulated_steps.append(f"Error: Timeout fetching URL {url}.")
    except requests.RequestException as e:
        logger.error(f"Error fetching URL {url}: {e}")
        error_message = f"Could not fetch content from URL: {e}"
        simulated_steps.append(f"Error: Could not fetch content from URL {url}. Details: {e}")
    except Exception as e: # Catch-all for other unexpected errors
        logger.error(f"An unexpected error occurred during processing: {e}", exc_info=True) # Log stack trace
